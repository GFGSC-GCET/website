  ...
  return(
    <>
        <div class="w-full max-w-xs m-5">
          <button href="#" class="flex items-center justify-center w-full text-gray-600 transition-colors duration-300 transform border rounded-lg dark:border-gray-700 dark:text-gray-200 hover:bg-gray-50 dark:hover:bg-gray-600" onClick={()=>{loginWithGoogle()}}>
            <div class="px-4 py-2">
...

        {
          ...
          <>
            <div class="w-full max-w-xs overflow-hidden bg-white rounded-lg shadow-lg dark:bg-gray-800 m-2">
                <img class="object-cover w-full h-56" src={user.photoURL} alt="avatar"/>

                <div class="py-5 text-center">
                    <a href="#" class="block text-2xl font-bold text-gray-800 dark:text-white" tabindex="0" role="link">{user.displayName}</a>
                    <span class="text-sm text-gray-700 dark:text-gray-200">{user.email}</span>
                </div>
            </div>
            <p class="text-sm font-light text-center text-gray-400 m-3"> This is not me.{" "}
            <button href="#" class="font-medium text-gray-700 dark:text-gray-200 hover:underline" onClick={()=>{logout();loginWithGoogle();}}>Change Account</button></p>
          </>
        
        }
        
    </div>
    { user != null && <Button click={()=>{router.replace('/join/complete')}}> Register Me </Button> }
